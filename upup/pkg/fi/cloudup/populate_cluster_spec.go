...
func (c *populateClusterSpec) registerInstanceGroupWarmPoolImages(cluster *kopsapi.Cluster) error {
	...
	if defaultWarmPool != nil && len(defaultWarmPool.AdditionalImages) > 0 {
		for _, image := range defaultWarmPool.AdditionalImages {
			if image != "" {
	...
	for _, ig := range c.InputInstanceGroups {
		...
		if len(resolvedWarmPool.AdditionalImages) > 0 {
			for _, image := range resolvedWarmPool.AdditionalImages {
				if image != "" {
