...
In some cases it can be convenient to download large container images during the warming phase to reduce the time for a workload to execute once the node is started. Extra images can be specified via the `warmPoolExtraImages` field.
...
By default the lifecycle hook will timeout after 600s. A custom timeout can be set via the `lifecycleHookTimeout` field in the `warmPool` spec.
